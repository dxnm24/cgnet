			...
			<div class="box">
				<ul class="box-tabs clearfix" data-tabs id="box-tabs-{{ $value->id }}">
					<li class="tabs-title is-active">
						<h3><a href="#{{ $value->slug.'-moi-nhat' }}" aria-selected="true" rel="nofollow">{!! $value->name !!} mới nhất</a></h3>
					</li>
					<li class="tabs-title">
						<h3><a href="#{{ $value->slug.'-hay-nhat' }}" rel="nofollow">{!! $value->name !!} hay nhất</a></h3>
					</li>
				</ul>
				<div class="box-inner" data-tabs-content="box-tabs-{{ $value->id }}">
					<div class="tabs-panel is-active" id="{{ $value->slug.'-moi-nhat' }}">
						<a href="{{ $url.'-moi-nhat' }}" class="btn-seemore float-right hide-for-small-only hvr-icon-wobble-horizontal hvr-buzz-out" rel="nofollow">Xem thêm</a>
						<div class="clearfix"></div>
						@include('site.game.box', array('data' => $value->games))
					</div>
					<div class="tabs-panel" id="{{ $value->slug.'-hay-nhat' }}">
						<a href="{{ $url.'-hay-nhat' }}" class="btn-seemore float-right hide-for-small-only hvr-icon-wobble-horizontal hvr-buzz-out" rel="nofollow">Xem thêm</a>
						<div class="clearfix"></div>
